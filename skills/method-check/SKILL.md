...
- **Futures Cone** — Scenarios across probable/plausible/possible/preposterous → indicators → robustness. Sees the possibility space, blind to *unknown unknowns* (the cone draws on imaginable futures; the most disruptive futures are usually outside what felt plausible enough to draw). Cannot detect when the four zones systematically share an unstated assumption (e.g. all four assume the institution survives, all four assume the regulatory regime is stable). Robustness analysis assumes the scenarios are exhaustive — they aren't.
